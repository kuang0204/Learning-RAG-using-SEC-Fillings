...
Key findings
Document dominance. A two-company comparison query collapsed into a single blended query embedding, so one company's language dominated dense retrieval and the other got zero chunks in the top 10. Fixed with query decomposition and round-robin merging of sub-query results.
